...
class Crawling(object):
    ...
    def __init__(self, userName, passward):
        # public data
        self.originURL = 'http://***/(0hn1iorbybwvx1v4nrmpit45)/'  # TODO 教务首页url
    ...
    def setSwitchToGradeData(self, URL, headers, year, semester):
        ...
        data['__VIEWSTATE'] = BeautifulSoup(self.session.get(URL, headers=headers).text, 'lxml').find('input', type='hidden', id=False).get('value')  #解析得到'__VIEWSTATE'的值, 将'__VIEWSTATE'的值加入字典

        return data
    
    def setSwitchToBlankClassroomData(self, URL, headers):
        data = {
            '__EVENTTARGET':'', 
            '__EVENTARGUMENT': '', 
            'xiaoq':'', 'jslb':'', 'min_zws': '0', 'max_zws':'', 
            'kssj': '', 'jssj': '', 'xqj': '', 'ddlDsz':'',
            'sjd': '',
            'xn': '', 'xq': '', 'ddlSyXn': '', 'ddlSyxq': '',
            }
        data['__VIEWSTATE'] = BeautifulSoup(self.session.get(URL, headers=headers).text, 'lxml').find('input', type='hidden', id=False).get('value')  #解析得到'__VIEWSTATE'的值, 将'__VIEWSTATE'的值加入字典
    ...
    def switchToBlankClassroom(self):
#        blankClassroomPageURL = self.originURL+'xxjsjy.aspx?xh='+self.__originData['txtUserName']+'&xm='+self.__nameInURL+'&gnmkdm=N121611'  #取得post网址, post请求网址, 同时也是referer网址
        blankClassroomPageURL = 'http://***/(fuzy3oq05iz4demerlx1qz55)/'+'xxjsjy.aspx?xh='+self.__originData['txtUserName']+'&xm='+self.__nameInURL+'&gnmkdm=N121611'  #取得post网址, post请求网址, 同时也是referer网址  # TODO 教务首页
        switchToBlankClassroomHeaders = self.setHeaders(blankClassroomPageURL)  #取得headers
        switchToBlankClassroomData = self.setSwitchToBlankClassroomData(blankClassroomPageURL, switchToBlankClassroomHeaders)  #取得data:
        blankClassroomPage = self.session.post(blankClassroomPageURL, data=switchToBlankClassroomData, headers=switchToBlankClassroomHeaders)
        
        return blankClassroomPage
